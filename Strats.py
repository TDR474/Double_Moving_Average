...
def market_open(context):
    ...
    # 如果上一时间点价格高出10天平均价, 则全仓买入
    if (current_price > MA10) and (cash > 0):
        # 记录这次买入
        log.info("价格高于均价 1%%, 买入 %s" % (security))
        print("当前可用资金为{0}, position_value为{0}".format(cash, context.portfolio.positions_value))
        # 用所有 cash 买入股票
        order_value(security, cash)
    # 如果上一时间点价格低于10天平均价, 则空仓卖出
    elif current_price < MA10 and context.portfolio.positions[security].closeable_amount > 0:
        # 记录这次卖出
